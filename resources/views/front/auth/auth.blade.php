...
@section('title')
	@switch($loginState)
		@case(\App\Enums\ELoginStage::NOT_REGISTERED)
			Đăng Ký
			@break
		@case(\App\Enums\ELoginStage::NOT_LOGGED_IN)
			Đăng nhập
			@break
		@case(\App\Enums\ELoginStage::FORGOT_PASSWORD_EMAIL)
			Quên mật khẩu
			@break
		@case(\App\Enums\ELoginStage::RESET_PASSWORD)
			Đặt lại mật khẩu
			@break
	@endswitch
@endsection
				...
				<div class="row form-login bg-white my-4 pt-4" style="box-shadow: 1px 1px 5px 1px #E0E0E0">
				    ...
				    <div class="col-48 py-3">
				        <div class="row">
				            ...
				            <div class="col-md-48 mb-4 text-center">
								<template v-if="stage === ELoginStage.FORGOT_PASSWORD_EMAIL">
									@lang('front/auth.auth-area.forgot-password-guide')
								</template>
								<template v-if="stage === ELoginStage.VERIFY_OTP_FORGOT_PASSWORD">
				            ...
				            <div class="col-md-48" v-for="field in formFields">
				                <div class="row mb-4" v-if="field.name === 'otp'">
									...
									<div class="invalid-feedback d-block mb-2 ml-2 font-medium">@{{ field.error }}</div>
				                ...
				                <div v-else class="form-group position-relative" style="border: 1px solid #0000000d; border-radius: 5px">
				                    ...
				                    <span v-if="field.name === 'password'" class="position-absolute" style="top: 32px; right: 10px">
                                        <i
											class="fas text-black-50 cursor-pointer"
											:class="{'fa-eye': field.type === 'text', 'fa-eye-slash': field.type === 'password'}"
											@click="formData.password.type = field.type === 'text' ? 'password' : 'text'"
										></i>
                                    </span>
                                    <div class="invalid-feedback d-block mb-2 ml-2 font-medium">@{{ field.error }}</div>
				                </div>
...
@push('app-scripts')
	<script src="{{ mix('/js/front/auth/auth.js') }}"></script>
@endpush
